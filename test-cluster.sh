...
NUM_NODES=2 KUBE_VERSION=v1.1.1 KUBERNETES_PROVIDER=vagrant NETWORK_PROVIDER=calico kubernetes/cluster/kube-up.sh

WORKERS=2; sed -i "s/NUM_NODES=[0-9]/NUM_NODES=${WORKERS}/" kubernetes/hack/conformance-test.sh
KUBECONFIG=/var/lib/jenkins/.kube/config kubernetes/hack/conformance-test.sh 2>&1 | tee conformance.$(date +%FT%T%z).log

NUM_NODES=2 KUBE_VERSION=v1.1.1 KUBERNETES_PROVIDER=vagrant NETWORK_PROVIDER=calico kubernetes/cluster/kube-down.sh
